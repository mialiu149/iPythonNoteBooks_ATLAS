...
nominalCR={'nonTightPh':'o_m_ph_nontight_def1','nonIsoPhCut':6,'nonIsoElCut':0.1,'highMET':35,'highMt':40,'lowMET':20,'lowMt':20}
nominalCR_2jet={'nonTightPh':'o_m_ph_nontight_def1','nonIsoPhCut':6,'nonIsoElCut':0.1,'highMET':35,'highMt':40,'lowMET':20,'lowMt':20}

def Inclusive_sig(csvfile=default_csv,CR=nominalCR): 
    sample = DefVars(csv_file=csvfile)
    sample = sample[np.where(sample['MET']>CR['highMET'])]
    sample = sample[np.where(sample['Mt']>CR['highMt'])]
    ...
    return sample
def Inclusive_B(csvfile=default_csv,CR=nominalCR):    
    sample = DefVars(csv_file = csvfile)
    sample = sample[np.where(sample['MET']>CR['highMET'])]
    sample = sample[np.where(sample['Mt']>CR['highMt'])]
    sample = sample[np.where(sample['lep_iso']/sample['lep_Pt'] < 0.1)]
    sample = sample[np.where(sample['ph_istight']==1)]
    sample = sample[np.where(sample['ph_iso_cone40'] > CR['nonIsoPhCut'])]
    return sample
def Inclusive_C(csvfile=default_csv,CR=nominalCR):    
    sample = DefVars(csv_file = csvfile)
    sample = sample[np.where(sample['MET']>CR['highMET'])]
    sample = sample[np.where(sample['Mt']>CR['highMt'])]
    sample = sample[np.where(sample['lep_iso']/sample['lep_Pt'] < 0.1)]
    sample = sample[np.where(sample[CR['nonTightPh']]>-1)]
    sample = sample[np.where(sample[CR['nonTightPh']])]
    sample = sample[np.where(sample['ph_iso_cone40'] < 4)]
    return sample
def Inclusive_D(csvfile=default_csv,CR=nominalCR):    
    sample = DefVars(csv_file = csvfile)
    sample = sample[np.where(sample['MET']>CR['highMET'])]
    sample = sample[np.where(sample['Mt']>CR['highMt'])]
    sample = sample[np.where(sample['lep_iso']/sample['lep_Pt'] < 0.1)]
    ...
    sample = sample[np.where(sample['ph_iso_cone40'] > CR['nonIsoPhCut'])]
    return sample
def Inclusive_Bprime(csvfile=default_csv,CR=nominalCR):    
    sample = DefVars(csv_file = csvfile)
    sample = sample[np.where(sample['MET']>CR['highMET'])]
    sample = sample[np.where(sample['Mt']>CR['highMt'])]
    sample = sample[np.where(sample['lep_iso']/sample['lep_Pt'] > CR['nonIsoElCut'])]
    sample = sample[np.where(sample['ph_istight'])]
    sample = sample[np.where(sample['ph_iso_cone40'] < 4)]
    return sample
def Inclusive_Cprime(csvfile=default_csv,CR=nominalCR): 
    sample = DefVars(csv_file = csvfile)
    sample = sample[np.where(sample['MET']<CR['lowMET'])]
    sample = sample[np.where(sample['Mt']<CR['lowMt'])]
    sample = sample[np.where(sample['lep_iso']/sample['lep_Pt'] < 0.1)]
    sample = sample[np.where(sample['ph_istight'])]
    sample = sample[np.where(sample['ph_iso_cone40'] < 4)]
    return sample
def Inclusive_Dprime(csv_file=default_csv,CR=nominalCR): 
    sample = DefVars(csv_file = csvfile)
    sample = sample[np.where(sample['MET']<CR['lowMET'])]
    sample = sample[np.where(sample['Mt']<CR['lowMt'])]
    sample = sample[np.where(sample['lep_iso']/sample['lep_Pt'] > CR['nonIsoElCut'])]
...
def CalComp(samples={'data':None,'wenugvbs':None,'wenug':None,'wenu':None,'zee':None,'dp':None,'ttbar':None},norms={'data':1,'wenugvbs':1,'wenug':1,'wenu':24769592/1.540386934e+15,'zee':1,'dp':1,'ttbar':1,'tg':1},CR={'nonTightPh':'o_m_ph_nontight_def1','nonIsoPhCut':6,'nonIsoElCut':0.1,'highMET':35,'highMt':40,'lowMET':20,'lowMt':20},output_report='default_output.txt'):        
    ...
    for key in keys:
        ...
        C[key] = ph_cr[key][np.where(ph_cr[key][CR['nonTightPh']]>-1)]# loose photon
        C[key] = C[key][np.where(C[key][CR['nonTightPh']])]# loose photon
        C[key] = C[key][np.where(C[key]['ph_iso_cone40'] < 4)] # Iso
        D[key] = ph_cr[key][np.where(ph_cr[key][CR['nonTightPh']]>-1)]# loose photon exists
        D[key] = D[key][np.where(D[key][CR['nonTightPh']])]# loose photon
    ...
    C_dp_forsigleak = samples['dp'][np.where(samples['dp'][CR['nonTightPh']])]
    ...
    D_dp_forsigleak = samples['dp'][np.where(samples['dp'][CR['nonTightPh']])]
...
#    C_D = (ND['wenug']+ND_dp_forsigleak)/(NA['wenug']+NA_dp_forsigleak)
    ...
    #f = open(output_report,'rt', encoding='utf-8')
    print 'Signal and background components: '
    print "Sample & NA & NB & NC & ND & NB' &NC' & ND'"
    f.write("Inclusive analysis in the electron channel") 
    f.write(r"\begin{table}[!htbp]") 
    f.write(r"\begin{tabular}{cccccccc}") 
    f.write('\hline\hline ')
    f.write(" Process & A & B &C &D &B'&C'&D'"+r'\\')
    f.write('\hline ')
    for key in keys:
    ...
    for key in keys:
        key_latex=key.replace('wenu',r'$W(e\nu)$')
        key_latex=key_latex.replace('ttbar',r'$t\bar{t}$')
        key_latex=key_latex.replace('g','$\gamma$')

        s = str(key_latex+' & $'+'{:2fL}'.format(NA[key])+'$&$'+'{:2fL}'.format(NB[key])+'$&$'+'{:2fL}'.format(NC[key])+'$&$'+'{:2fL}'.format(ND[key])+'$&$'+'{:2fL}'.format(NBprime[key])+'$&$'+'{:2fL}'.format(NCprime[key])+'$&$'+'{:2fL}'.format(NDprime[key])+'$'+r'\\')
        s.replace('dp','$\gamma$+jets')
        s.replace('wenu',r'$W(e\nu)$')
        s.replace('ttbar',r'$t\bar{t}$')
        f.write(s)
    ...
    s_wenu = str('wenu DD &'+'{:.4f}'.format(NA_wenu_DD)+', wenu MC : '+'{:.4f}'.format(NA['wenu']))
    s_dp = str('dp DD &'+'{:.4f}'.format(NA_dp_DD)+', dp MC : '+'{:.4f}'.format(NA['dp']))
    dd_parameters  = "R & $C_B$ &$C_C$ &$C_D$ &R' & $C_B'$ & $C_C'$&$C_D'$"+r'\\'
    s_dd_parameters = str('${:4fL}'.format(NA['wenu']*ND['wenu']/(NC['wenu']*NB['wenu']))+'$&$'+'{:4fL}'.format(C_B)+'$&$'+'{:4fL}'.format(C_C)+'$&$'+'{:4fL}'.format(C_D)+'$&${:4fL}'.format(NA['wenu']*NDprime['dp']/(NCprime['dp']*NBprime['dp']))+'$&$'+'{:4fL}'.format(C_Bprime)+'$&$'+'{:4fL}'.format(C_Cprime)+'$&$'+'{:4fL}'.format(C_Dprime)+r'$\\')
    ...
    f.write(' \hline ')
    f.write(' &&&Data driven method &&&& '+r'\\')
    f.write(' \hline ')
    f.write(' &&&MC closure and signal leakage parameters &&&&'+r'\\')
    f.write(' \hline ')
    f.write(dd_parameters)
    f.write(s_dd_parameters)
    f.write(' \hline ')
    f.write('&&&Results&&&&'+r'\\')
    f.write(' \hline ')
    f.write('&W+jets&&$\gamma$+jets&&Signal yield&'+r'\\')
    s_results = '&${:.1fL}'.format(NA_wenu_DD)+'$&&${:.1fL}'.format(NA_dp_DD)+'$&&${:.1fL}'.format(components['signalyield'])+r'$\\'
    f.write(s_results)
    f.write(' \hline ')
